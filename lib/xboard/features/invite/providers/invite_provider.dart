...
class InviteNotifier extends Notifier<InviteState> {
  ...
  Future<void> loadInviteData() async {
    ...
    try {
      _logger.info('加载邀请信息...');
      final inviteData = await ref.read(getInviteInfoProvider.future);
  ...
  Future<void> loadCommissionHistory({int page = 1, bool append = false}) async {
    ...
    try {
      ...
      final newHistory = await ref.read(getCommissionDetailsProvider(page: page).future);
  ...
  Future<DomainInviteCode?> generateInviteCode() async {
    ...
    try {
      ...
      final newCode = codes.isNotEmpty ? codes.last : null;
  ...
  Future<bool> withdrawCommission({
    ...
    try {
      ...
      final api = await ref.read(xboardSdkProvider.future);
      await api.withdrawTicket(withdrawMethod, withdrawAccount);
  ...
  Future<bool> transferCommission(double amount) async {
    ...
    try {
      ...
      final api = await ref.read(xboardSdkProvider.future);
      // V2Board transfer expects amount in cents (int)
      await api.transfer((amount * 100).toInt());
