...
function index()
	...
	entry({"admin", "services", "openclash", "xboard", "config"}, call("action_xboard_config")).leaf = true
	entry({"admin", "services", "openclash", "xboard", "proxy"}, call("action_xboard_proxy")).leaf = true
	entry({"admin", "services", "openclash", "client"},form("openclash/client"),nil).leaf = true
	...
	entry({"admin", "services", "openclash", "settings"},cbi("openclash/settings"),nil).leaf = true
	...
	entry({"admin", "services", "openclash", "rule-providers-settings"},cbi("openclash/rule-providers-settings"),nil).leaf = true
	...
	entry({"admin", "services", "openclash", "config-subscribe"},cbi("openclash/config-subscribe"),nil).leaf = true
	...
	entry({"admin", "services", "openclash", "config"},form("openclash/config"),nil).leaf = true
	entry({"admin", "services", "openclash", "log"},cbi("openclash/log"),nil).leaf = true
...
function action_add_subscription()
	...
	local provider_type = luci.http.formvalue("provider_type") or "default"
	local provider_auth_ref = luci.http.formvalue("provider_auth_ref") or "auto"
	local xboard_email = luci.http.formvalue("xboard_email") or ""
	local xboard_password = luci.http.formvalue("xboard_password") or ""
	local xboard_token = luci.http.formvalue("xboard_token") or ""
	local xboard_sub_path = luci.http.formvalue("xboard_sub_path") or "/api/v1/client/subscribe?token={token}"
	local enc_enable = luci.http.formvalue("enc_enable") or "0"
	local enc_format = luci.http.formvalue("enc_format") or "json-envelope"
	local enc_algo = luci.http.formvalue("enc_algo") or "aes-256-cbc"
	local enc_key_ref = luci.http.formvalue("enc_key_ref") or "token-sha256"
	local enc_meta = luci.http.formvalue("enc_meta") or "iv=iv,data=data,tag=tag,cipher=cipher,format=format"
	local enc_key_value = luci.http.formvalue("enc_key_value") or ""
	...
	if provider_type ~= "xboard" and (not address or address == "") then
		luci.http.write_json({
			status = "error",
	...
	if not is_valid_url then
		local error_msg
		if provider_type == "xboard" then
			error_msg = "Invalid xboard subscription configuration"
		elseif sub_convert == "1" then
	...
	if provider_type == "xboard" then
		normalized_address = (address or ""):match("^%s*(.-)%s*$")
	elseif sub_convert == "1" and (string.find(address, "\n") or string.find(address, "|")) then
		local links = {}
		if string.find(address, "\n") then
			for line in address:gmatch("[^\n]+") do
	...
	if section_id then
		...
		uci:set("openclash", section_id, "provider_type", provider_type)

		if provider_type == "xboard" then
			uci:set("openclash", section_id, "provider_auth_ref", provider_auth_ref)
			uci:delete("openclash", section_id, "xboard_base_url")
			uci:set("openclash", section_id, "xboard_email", xboard_email)
			uci:set("openclash", section_id, "xboard_password", xboard_password)
			uci:set("openclash", section_id, "xboard_token", xboard_token)
			uci:set("openclash", section_id, "xboard_sub_path", xboard_sub_path)
			uci:set("openclash", section_id, "enc_enable", enc_enable)
			uci:set("openclash", section_id, "enc_format", enc_format)
			uci:set("openclash", section_id, "enc_algo", enc_algo)
			uci:set("openclash", section_id, "enc_key_ref", enc_key_ref)
			uci:set("openclash", section_id, "enc_meta", enc_meta)
			uci:set("openclash", section_id, "enc_key_value", enc_key_value)
		else
			uci:delete("openclash", section_id, "provider_auth_ref")
			uci:delete("openclash", section_id, "xboard_base_url")
			uci:delete("openclash", section_id, "xboard_email")
			uci:delete("openclash", section_id, "xboard_password")
			uci:delete("openclash", section_id, "xboard_token")
			uci:delete("openclash", section_id, "xboard_sub_path")
			uci:delete("openclash", section_id, "enc_enable")
			uci:delete("openclash", section_id, "enc_format")
			uci:delete("openclash", section_id, "enc_algo")
			uci:delete("openclash", section_id, "enc_key_ref")
			uci:delete("openclash", section_id, "enc_meta")
			uci:delete("openclash", section_id, "enc_key_value")
		end
...
local function xboard_trim(value)
	if not value then
		return ""
	end
...
local function xboard_trim_url(value)
	value = xboard_trim(value)
...
local function xboard_valid_http_url(value)
	value = xboard_trim_url(value)
...
local function xboard_shell_quote(value)
	return luci.util.shellquote(tostring(value or ""))
end

local function xboard_load_constants()
	local defaults = {
		API_BASE_URL = "",
		API_TEXT_DOMAIN = "",
		APP_ICON_URL = "",
...
local function xboard_cleanup_legacy_base_url()
	local changed = false
	uci:foreach("openclash", "config_subscribe", function(s)
		if s.xboard_base_url ~= nil then
			uci:delete("openclash", s[".name"], "xboard_base_url")
			changed = true
		end
	end)
	if changed then
...
local xboard_api_text_cache = {}
local xboard_base_race_cache = {
	key = "",
	base_url = "",
	expire = 0
}

local function xboard_force_https_url(value)
	value = xboard_trim_url(value)
	if value == "" then
		return ""
	end
...
local function xboard_append_unique(tbl, value)
	if type(tbl) ~= "table" then
		return
	end
	value = xboard_valid_http_url(value)
	if value == "" then
		return
	end
	value = xboard_force_https_url(value)
...
local function xboard_resolve_api_text_hosts(constants)
	constants = constants or {}
	local api_text_domain = xboard_trim(constants.API_TEXT_DOMAIN)
	local password = xboard_trim(constants.APP_NAME)
	...
	if xboard_api_text_cache[cache_key] then
		return xboard_api_text_cache[cache_key]
	end

	if api_text_domain:match("^https?://") then
		local direct = { xboard_force_https_url(api_text_domain) }
		xboard_api_text_cache[cache_key] = direct
		return direct
	end
...
begin
  ...
  servers.each do |url|
    workers << Thread.new do
      begin
        ...
        req['user-agent'] = 'openclash-xboard-ui/1.0'
        ...
        if local_txt && !local_txt.empty?
          mutex.synchronize { txt ||= local_txt }
        end
      rescue
      end
    end
  ...
  hosts = config['hosts']
  exit 1 unless hosts.is_a?(Array)
  hosts.each do |host|
  ...
  exit 1
	...
	local command = "ruby -rjson -ruri -rnet/http -rbase64 -ropenssl -rdigest -rthread -e " ..
		xboard_shell_quote(ruby_code) .. " " ..
		xboard_shell_quote(api_text_domain) .. " " ..
		xboard_shell_quote(password) .. " 2>/dev/null"
	local output = luci.sys.exec(command) or ""

	local hosts = {}
	for line in output:gmatch("[^\r\n]+") do
		local host = xboard_trim(line)
		if host ~= "" then
			if not host:match("^https?://") then
				host = "https://" .. host
			end
			xboard_append_unique(hosts, host)
		end
	end

	if #hosts == 0 and type(constants.TEXT_DOMAIN_MAP) == "table" then
		local mapped = constants.TEXT_DOMAIN_MAP[api_text_domain]
		xboard_append_unique(hosts, mapped)
	end

	xboard_api_text_cache[cache_key] = hosts
	return hosts
end

local function xboard_race_base_url(candidates)
	...
	local script = "rm -f " .. xboard_shell_quote(tmp_file) .. "; "

	for _, base in ipairs(candidates) do
		local test_url = xboard_trim_url(base) .. "/api/v1/guest/comm/config"
		script = script ..
			"( r=$(curl -k -sS --noproxy '*' --connect-timeout 5 -m 8 -o /dev/null -w '%{http_code} %{time_total}' " ..
			xboard_shell_quote(test_url) .. " 2>/dev/null); " ..
			"c=${r%% *}; t=${r#* }; case \"$c\" in 2*|3*) printf '%s\\t%s\\n' \"$t\" " ..
			xboard_shell_quote(base) .. " >> " .. xboard_shell_quote(tmp_file) .. ";; esac ) & "
	end

	script = script ..
		"wait; " ..
		"if [ -s " .. xboard_shell_quote(tmp_file) .. " ]; then sort -n " .. xboard_shell_quote(tmp_file) ..
		" | head -n1 | awk '{print $2}'; fi; " ..
		"rm -f " .. xboard_shell_quote(tmp_file)

	local winner = xboard_trim(luci.sys.exec(script) or "")
	if winner ~= "" then
		return winner
	end
	return candidates[1]
...
local function xboard_resolve_base_url()
	local constants = xboard_load_constants()
	local cache_key = table.concat({
		xboard_trim(constants.API_BASE_URL),
		xboard_trim(constants.API_TEXT_DOMAIN),
		xboard_trim(constants.APP_NAME)
	}, "|")
	local now = os.time()

	if xboard_base_race_cache.key == cache_key and xboard_base_race_cache.expire > now and xboard_base_race_cache.base_url ~= "" then
		return xboard_base_race_cache.base_url, constants
	end

	local candidates = {}
	xboard_append_unique(candidates, constants.API_BASE_URL)

	local api_text_hosts = xboard_resolve_api_text_hosts(constants)
	for _, host in ipairs(api_text_hosts) do
		xboard_append_unique(candidates, host)
	end
	...
	local winner = xboard_race_base_url(candidates)
	xboard_base_race_cache.key = cache_key
	xboard_base_race_cache.base_url = winner
	xboard_base_race_cache.expire = now + 300
	return winner, constants
end

local function xboard_build_query_string(query_tbl)
	if type(query_tbl) ~= "table" then
		return ""
	end

	local ok_http, http = pcall(require, "luci.http")
	if not ok_http or not http or not http.urlencode then
		return ""
	end

	local pairs_list = {}
	for key, value in pairs(query_tbl) do
		if key ~= nil and value ~= nil then
			pairs_list[#pairs_list + 1] = http.urlencode(tostring(key)) .. "=" .. http.urlencode(tostring(value))
		end
	end
...
local function xboard_parse_proxy_payload()
	local body_json = luci.http.formvalue("data") or ""
	if body_json == "" then
		return ""
	end

	local parsed = json.parse(body_json)
	if parsed == nil then
		return body_json
	end
...
local function xboard_http_request(method, request_url, auth_data, payload)
	...
	if method == "POST" then
		cmd = cmd .. " -H " .. xboard_shell_quote("Content-Type: application/json")
		cmd = cmd .. " --data " .. xboard_shell_quote(payload or "{}")
	end

	cmd = cmd ..
		" -w " .. xboard_shell_quote("\\n%{http_code}") ..
		" " .. xboard_shell_quote(request_url) ..
		" 2>/dev/null"

	local output = luci.sys.exec(cmd) or ""
	...
	local parsed = json.parse(body or "")
	return status_code, body or "", parsed
end

local function xboard_proxy_error(message, status)
	luci.http.prepare_content("application/json")
	luci.http.write_json({
		ok = false,
...
function action_xboard_proxy()
	...
	if method ~= "GET" and method ~= "POST" then
		return xboard_proxy_error("Unsupported method", 405)
	end
	if path == "" then
		return xboard_proxy_error("Missing API path", 400)
	end
	if not path:match("^/api/v1/") then
		return xboard_proxy_error("API path must start with /api/v1/", 400)
	end
	if path:find("%.%.", 1, true) then
		return xboard_proxy_error("Invalid API path", 400)
	end

	local base_url = select(1, xboard_resolve_base_url())
	if not base_url or base_url == "" then
		return xboard_proxy_error("XBoard API base URL is not configured", 503)
	end

	local query_tbl = nil
	if query_json ~= "" then
		query_tbl = json.parse(query_json)
		if query_tbl ~= nil and type(query_tbl) ~= "table" then
			query_tbl = nil
		end
	end

	local query_string = xboard_build_query_string(query_tbl)
	...
	luci.http.write_json({
		...
		status = status_code,
		data = parsed,
		raw = parsed and nil or raw_body
	})
end
