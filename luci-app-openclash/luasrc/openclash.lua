...
local function _valid_http_url(url)
	url = _trim(url)
	if url == "" then
		return ""
	end
...
local function _valid_theme_color(color)
	color = _trim(color)
	if color == "" then
		return ""
	end
	if color:match("^#%x%x%x$") or color:match("^#%x%x%x%x%x%x$") then
		return color:upper()
	end
	return ""
end
...
function get_branding()
	...
	if ok and type(constants) == "table" then
		...
		local theme_color = _valid_theme_color(constants.THEME_COLOR)
		local crisp_id = _trim(constants.CRISP_WEBSITE_ID)
		local package_name = _trim(constants.APP_PACKAGE_NAME)

		if app_name ~= "" then
			branding.app_name = app_name
		end
		branding.app_icon_url = icon_url
		branding.theme_color = theme_color
		branding.crisp_website_id = crisp_id
		branding.app_package_name = package_name

		if crisp_id ~= "" then
			local crisp_query = crisp_id
			local ok_http, http = pcall(require, "luci.http")
			if ok_http and http and http.urlencode then
				crisp_query = http.urlencode(crisp_id)
			end
			branding.crisp_chat_url = "https://go.crisp.chat/chat/embed/?website_id=" .. crisp_query
		end

		branding.show_branding = (
			branding.app_name ~= "OpenClash" or
			branding.app_icon_url ~= "" or
			branding.theme_color ~= "" or
			branding.crisp_chat_url ~= "" or
			branding.app_package_name ~= ""
		)
	end
